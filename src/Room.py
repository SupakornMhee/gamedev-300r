...
class Room:
    ...
    def GenerateEntities(self):
        ...
        while len(self.entities) < NUMBER_OF_MONSTER:
            ...
            def dummyPlayer(x,y) :
                return EntityBase(EntityConf(animation=gPlayer_animation_list, walk_speed=PLAYER_WALK_SPEED,
                         x=x, y=y, width=48*1.5, height=66*1.5,
                         health=6, offset_x=0, offset_y=15))
            
            if EntityBase(conf).Collides(dummyPlayer(552,294)) or \
                EntityBase(conf).Collides(dummyPlayer(552,96)) or \
                EntityBase(conf).Collides(dummyPlayer(552,470)) or \
                EntityBase(conf).Collides(dummyPlayer(99,294)) or \
                EntityBase(conf).Collides(dummyPlayer(1008,294)) :
                    continue
            self.entities.append(EntityBase(conf))

            self.entities[-1].state_machine = StateMachine()
            self.entities[-1].state_machine.SetScreen(pygame.display.get_surface())
            self.entities[-1].state_machine.SetStates({
                "walk": EntityWalkState(self.entities[-1]),
                "idle": EntityIdleState(self.entities[-1])
            })
    ...
    def GenerateObjects(self):
        ...
        switch_x = random.randint(MAP_RENDER_OFFSET_X + TILE_SIZE, WIDTH-TILE_SIZE*2 - 48)
        switch_y = random.randint(MAP_RENDER_OFFSET_Y+TILE_SIZE, HEIGHT-(HEIGHT-MAP_HEIGHT*TILE_SIZE) + MAP_RENDER_OFFSET_Y - TILE_SIZE - 48)
        ...
        '''
        pot_x = random.randint(MAP_RENDER_OFFSET_X + TILE_SIZE, WIDTH-TILE_SIZE*2 - 48)
        pot_y = random.randint(MAP_RENDER_OFFSET_Y+TILE_SIZE, HEIGHT-(HEIGHT-MAP_HEIGHT*TILE_SIZE) + MAP_RENDER_OFFSET_Y - TILE_SIZE - 48) 
        while abs(pot_x-switch_x)<4 or abs(pot_x-switch_x)<4 : 
            pot_x = random.randint(MAP_RENDER_OFFSET_X + TILE_SIZE, WIDTH-TILE_SIZE*2 - 48)
            pot_y = random.randint(MAP_RENDER_OFFSET_Y+TILE_SIZE, HEIGHT-(HEIGHT-MAP_HEIGHT*TILE_SIZE) + MAP_RENDER_OFFSET_Y - TILE_SIZE - 48)
